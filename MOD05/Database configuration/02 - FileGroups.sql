...
ON PRIMARY (
   NAME = 'MyDB',
   FILENAME = N'C:\SQL\MyDB_Primary.mdf'),
FILEGROUP MyFileGroup (
   NAME = 'MyDBFileGroup2',
   FILENAME = N'c:\SQL\MyDB_MyFilegroup.ndf')
GO
...
IF OBJECT_ID('[dbo].[utbMyTable1]') IS NOT NULL
   DROP TABLE [dbo].[utbMyTable1]
GO
...
ALTER TABLE [dbo].[utbMyTable1]
ADD CONSTRAINT [PK_utbMyTable1]
PRIMARY KEY CLUSTERED ([Col1])
GO

CREATE INDEX IX_utbMyTableCol2
ON [dbo].[utbMyTable1]([Col2])
ON [PRIMARY]
GO

ALTER TABLE [dbo].[utbMyTable1]
ADD CONSTRAINT [UQ_utbMyTable1_Col3]
UNIQUE([Col3])
ON [PRIMARY]
GO

--return the filegroup information for all indexes and constraints that belong to the table 
SELECT a.indid, a.groupid, a.name, b.groupname
FROM sysindexes a
   INNER JOIN sysfilegroups b
   ON a.groupid = b.groupid
WHERE a.id = OBJECT_ID('[dbo].[utbMyTable1]')
   AND INDEXPROPERTY(a.id, a.name, 'IsAutoStatistics') = 0;
GO

--Recreate the clustered index using the CREATE INDEX command together with the DROP_EXISTING clause
CREATE UNIQUE CLUSTERED INDEX [PK_utbMyTable1]
ON [dbo].[utbMyTable1] ([Col1])
WITH DROP_EXISTING
ON [MyFilegroup]
GO
